...
def _atomic_json(path: Path, payload: dict[str, Any]) -> None:
    """Keep packaging independent from mutable project business-module imports."""
...
def build_package(output_dir: Path) -> dict[str, Any]:
    ...
    _assert_no_link_components(output_dir)

    source_files = list(_source_files(output_dir))
    ...
    try:
        ...
        with zipfile.ZipFile(temporary) as archive:
            ...
            with archive.open(manifest_info) as stream:
                loaded_manifest = json.loads(stream.read(MAX_FILE_BYTES + 1))
    ...
    sidecar_path = (output_dir / f"{archive_name}.sha256").resolve()
    sidecar_path.relative_to(output_dir)
